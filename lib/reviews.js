...
export async function submitReview({ convenience, content, overall, comment }) {
  ...
  // Gắn danh tính người xem (từ cổng đăng ký) vào đánh giá nếu có.
  let visitor = null;
  if (hasWindow()) {
    try {
      const raw = localStorage.getItem('kw_visitor');
  ...
  const payload = {
    ...
    visitor_field: visitor?.field ?? null,
    visitor_birth_year: visitor?.birth_year ?? null,
  };
