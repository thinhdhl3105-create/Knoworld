...
const KIND_META = {
  ...
  image: { label: 'Image Case Study', href: '/images', icon: 'image' },
...
function matchesQuery(item, terms) {
  const haystack = [
    ...
    item.brand,
    item.author,
    Array.isArray(item.authors) ? item.authors.map((a) => a && a.name).filter(Boolean).join(' ') : '',
...
export async function searchAll(query) {
  ...
  const [videos, students, images, research, concepts] = await Promise.all([
    fetchContent('video'),
