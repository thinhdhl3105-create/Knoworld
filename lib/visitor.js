...
// info: { fullName, email, birthYear, field, role, studentId }
// role: 'student' (cần MSSV hợp lệ) | 'stranger' (khách — xem giới hạn)
export async function registerVisitor({ fullName, email, birthYear, field, role, studentId }) {
  const cleanRole = role === 'stranger' ? 'stranger' : 'student';
  const clean = {
    ...
    role: cleanRole,
    student_id: cleanRole === 'student' ? (studentId || '').trim().toUpperCase() || null : null,
  };

  if (!clean.full_name || !clean.email) {
    return { ok: false, error: 'Please enter your full name and email.' };
  }
  if (cleanRole === 'student' && !clean.student_id) {
    return { ok: false, error: 'Please enter your student ID (MSSV).' };
  }
...
// ---------------------------------------------------------------------------
// Vai trò & quyền truy cập (v18): student / stranger, full_access.
// ---------------------------------------------------------------------------

// Kiểm tra MSSV có trong danh sách được duyệt không (RPC, anon gọi được).
// Trả về { ok, fullName? , error? }.
export async function checkStudentId(studentId) {
  const id = (studentId || '').trim().toUpperCase();
  if (!id) return { ok: false, error: 'Please enter your student ID (MSSV).' };
  // Demo không có Supabase: cho qua để không chặn trải nghiệm.
  if (!isSupabaseConfigured) return { ok: true, fullName: null };

  const { data, error } = await supabase.rpc('check_student_id', { p_student_id: id });
  if (error) return { ok: false, error: error.message };
  const row = Array.isArray(data) ? data[0] : data;
  if (!row?.ok) {
    return {
      ok: false,
      error: 'Your student ID is not on the approved list. Please check it, or continue as a Guest.',
    };
  }
  return { ok: true, fullName: row.full_name || null };
}

// Vai trò hiện tại. Visitor cũ (trước v18) không có role -> coi là student.
export function getRole() {
  const v = getVisitor();
  if (!v) return null;
  return v.role === 'stranger' ? 'stranger' : 'student';
}

// Stranger chưa được admin allow access -> chỉ xem giới hạn 5 case.
export function isLimitedVisitor() {
  const v = getVisitor();
  return !!v && v.role === 'stranger' && !v.full_access;
}

// Hỏi server quyền mới nhất (admin có thể vừa bật full_access / block).
// Cập nhật localStorage và trả về { role, fullAccess, blocked }.
export async function syncAccess() {
  const v = getVisitor();
  if (!v || !isSupabaseConfigured) {
    return {
      role: getRole(),
      fullAccess: !!v?.full_access,
      blocked: isBlocked(),
    };
  }
  const { data, error } = await supabase.rpc('visitor_access', { p_anon_id: getAnonId() });
  if (error) {
    return { role: getRole(), fullAccess: !!v.full_access, blocked: isBlocked() };
  }
  const row = Array.isArray(data) ? data[0] : data;
  if (!row) {
    return { role: getRole(), fullAccess: !!v.full_access, blocked: isBlocked() };
  }
  const next = { ...v, role: row.role || v.role || 'student', full_access: !!row.full_access };
  saveVisitorLocal(next);
  if (hasWindow()) {
    if (row.blocked) localStorage.setItem(KEY_BLOCKED, '1');
    else localStorage.removeItem(KEY_BLOCKED);
  }
  return { role: next.role, fullAccess: next.full_access, blocked: !!row.blocked };
}

// Admin: cho stranger được xem full như student (cột full_access).
export async function setVisitorFullAccess(id, fullAccess) {
