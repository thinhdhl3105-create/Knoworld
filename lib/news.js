...
// v21: Hỏi server (/api/og) để lấy ảnh og:image của bài báo làm thumbnail.
// Trả về URL ảnh hoặc null (không chặn việc đăng tin nếu thất bại).
...
// Admin thêm tin.
export async function addNews({ title, url, year, category, summary, imageUrl, user }) {
  const t = (title || '').trim();
  const u = normalizeUrl(url);
  ...
  const payload = {
    ...
    image_url: (imageUrl || '').trim() || null, // v21: thumbnail
